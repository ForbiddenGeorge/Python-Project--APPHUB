...
class ToDo(ft.UserControl):
    def build(self):
        self.input = ft.TextField(hint_text='Nové úkoly', expand=True)
        self.tasks = ft.Column()

        view = ft.Column(
            horizontal_alignment=ft.CrossAxisAlignment.CENTER,
            controls=[
                ft.Text(value='To Do', style=ft.TextThemeStyle.HEADLINE_MEDIUM),
                ft.Row(
                    controls=[
                        self.input,
...
def ToDoPage(page, ft=ft):
    ToDo.input = ft.TextField(hint_text='Nové úkoly', expand=True)
    ToDo.tasks = ft.Column()
